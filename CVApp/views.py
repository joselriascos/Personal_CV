...
def home(request):
    ...
    referencias_laborales=Referencia_laboral.objects.all()
    referencias_personales=Referencia_personal.objects.all()
    referencias_familiares=Referencia_familiar.objects.all()
    return render(request, 'CVApp/home.html',{"instituciones":instituciones,
...
def proyectos(request):
    ...
    proyectos_paginados=paginator.get_page(numero_pagina) #Proyectos para esta página
    ...
    return render(request, 'CVApp/proyectos.html',
...
def contacto(request):
    if request.method == "POST":
        ...
        mail=EmailMessage("Mensaje desde portafolio",
				"El usuario {} con la dirección de correo {} escribe lo siguiente:\n\n {}".format(nombre,correo,mensaje),
				"",["joseluis.riascos10@gmail.com"],reply_to=[correo])
